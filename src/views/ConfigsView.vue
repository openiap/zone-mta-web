  ...
  <div class="card">
    <EasyDataTable :headers="headers" :items="items" :loading="loading" :server-items-length="serverItemsLength"
      ...
      <template #item-domains="item">
        <span :title="item.domains">{{ item.domains?.join(",").substring(0, 30) }}</span>
      </template>
      <template #item-emails="item">
        <span :title="item.emails">{{ item.emails?.join(",").substring(0, 30) }}</span>
      </template>
      <template #item-name="item">
        <router-link :to="{ name: 'ConfigWithId', params: { id: item._id }}">{{item.name}}
        </router-link>
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      searchfields: ['name', 'domains', 'emails'],
      items: [
      ],
      itemsSelected: [],
      timer: null,
  ...
  async created() {
    ...
    this.searchValue = await this.PageStateGet({key: "configs_" + this.collectionname + "_searchValue", defaultvalue: ""});
    this.serverOptions.rowsPerPage = await this.PageStateGet({key: "configs_" + this.collectionname + "_rowsPerPage", defaultvalue: 10});
    this.serverOptions.sortBy = await this.PageStateGet({key: "configs_" + this.collectionname + "_sortby", defaultvalue: "_created"});
    if(this.collectionname == "fs.files" && this.serverOptions.sortBy == "_created") {
      this.serverOptions.sortBy = "metadata._created"
    }
    this.serverOptions.sortType = await this.PageStateGet({key: "configs_" + this.collectionname + "_sorttype", defaultvalue: "desc"});
    this.GetData()
  },
  mounted() {
    this.FocusSearch();
  ...
  watch: {
    ...
    collectionname: async function (val, oldVal) {
      if (!Util.IsNullEmpty(val) == true && oldVal != val) {
        this.searchValue = await this.PageStateGet({key: "configs_" + this.collectionname + "_searchValue", defaultvalue: ""});
        this.PageStateSet({key: "configs_collection", value: val});
        // this.$router.push(`events/${val}`)
        // this.$router.replace({ path: `/events/${val}` })
        this.itemsSelected = [];
        this.serverItemsLength = 0;
        this.serverOptions.page = await this.PageStateGet({key: "configs_" + this.collectionname + "_page", defaultvalue: 1});
        this.serverOptions.sortBy = await this.PageStateGet({key: "configs_" + this.collectionname + "_sortby", defaultvalue: "_created"});
        if(this.collectionname == "fs.files" && this.serverOptions.sortBy == "_created") {
          this.serverOptions.sortBy = "metadata._created"
        }
  ...
  methods: {
    ...
    async DoSearch() {
      if (this.lastSearchValue != this.searchValue) {
        await this.PageStateSet({key: "configs_" + this.collectionname + "_searchValue", value: this.searchValue});
        this.serverItemsLength = 0;
    ...
    async Delete() {
      try {
        ...
        for(let i = 0; i < this.itemsSelected.length; i++) {
          ...
          await this.Client.DeleteMany({ collectionname: this.collectionname, query: { _id: item._id } });
        }
        this.GetData();
      } catch (error) {
    ...
    async GetData() {
      ...
      try {
        ...
        if(this.serverOptions.page == 1){
          await this.PageStateSet({key: "configs_" + this.collectionname + "_page", value: ""});
        } else {
          await this.PageStateSet({key: "configs_" + this.collectionname + "_page", value: this.serverOptions.page});
        }
        if(this.serverOptions.rowsPerPage == 10){
          await this.PageStateSet({key: "configs_" + this.collectionname + "_rowsperpage", value: ""});
        } else {
          await this.PageStateSet({key: "configs_" + this.collectionname + "_rowsperpage", value: this.serverOptions.page});
        }
        if(this.serverOptions.sortBy == "_created" && this.collectionname != "fs.files") {
          await this.PageStateSet({key: "configs_" + this.collectionname + "_sortby", value: ""});
        } else if(this.serverOptions.sortBy == "metadata._created" && this.collectionname == "fs.files") {
          await this.PageStateSet({key: "configs_" + this.collectionname + "_sortby", value: ""});
        } else {
          await this.PageStateSet({key: "configs_" + this.collectionname + "_sortby", value: this.serverOptions.sortBy});
        }
        if(this.serverOptions.sortType == "desc") {
          await this.PageStateSet({key: "configs_" + this.collectionname + "_sorttype", value: ""});
        } else {
          await this.PageStateSet({key: "configs_" + this.collectionname + "_sorttype", value: this.serverOptions.sortType});
        }

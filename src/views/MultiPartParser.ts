...
export class MultiPartParser {
    ...
    constructor(rawContent) {
        ...
        // Attachment
        const contentDisposition:any = this.getHeader('Content-Disposition');
     ...
     */
    getContentType() {
        let ct:any = this.getHeader('Content-Type');
        if (ct) {
    ...
    splitHeaderFromBody(arrbuf) {
        const arr = new Uint8Array(arrbuf), r = 0x0D, n = 0x0A; // \r\n suchen
        let separatorPos:any = null, separatorLength=0;
        for (let i = 0; i < arr.length; i++) {
        ...
        let headerArray:any = null;
        let bodyArray:any = null;
    ...
    parseBodyText(rawArray) {
        ...
        const arrayBuf = this.decodeContent(rawArray, charset as any);

        // Text always as utf-8
        const decoder = new TextDecoder();
        this.body = decoder.decode(new Uint8Array(arrayBuf)) as any;
    }

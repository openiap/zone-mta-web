  ...
  <div class="card">
    <EasyDataTable :headers="headers" :items="items" :loading="loading" :server-items-length="serverItemsLength"
      ...
      <template #item-id="item">
        <router-link v-if="item.id != '' && item.id != null" :to="{ name: 'QuarantineWithId', params: { id: item.id }}">{{item.id}}
        </router-link>
      </template>
    </EasyDataTable>
...
export default {
  ...
  async created() {
    this.collectionname = "mailevents"
    this.searchValue = await this.PageStateGet({key: "events_" + this.collectionname + "_searchValue", defaultvalue: ""});
    this.serverOptions.rowsPerPage = await this.PageStateGet({key: "events_" + this.collectionname + "_rowsPerPage", defaultvalue: 10});
    this.serverOptions.sortBy = await this.PageStateGet({key: "events_" + this.collectionname + "_sortby", defaultvalue: "_created"});
    if(this.collectionname == "fs.files" && this.serverOptions.sortBy == "_created") {
      this.serverOptions.sortBy = "metadata._created"
    }
    this.serverOptions.sortType = await this.PageStateGet({key: "events_" + this.collectionname + "_sorttype", defaultvalue: "desc"});
    this.GetData()
  },
  mounted() {
    this.FocusSearch();
  ...
  watch: {
    ...
    collectionname: async function (val, oldVal) {
      if (!Util.IsNullEmpty(val) == true && oldVal != val) {
        this.searchValue = await this.PageStateGet({key: "events_" + this.collectionname + "_searchValue", defaultvalue: ""});
        this.PageStateSet({key: "events_collection", value: val});
        // this.$router.push(`events/${val}`)
        // this.$router.replace({ path: `/events/${val}` })
        this.itemsSelected = [];
        this.serverItemsLength = 0;
        this.serverOptions.page = await this.PageStateGet({key: "events_" + this.collectionname + "_page", defaultvalue: 1});
        this.serverOptions.sortBy = await this.PageStateGet({key: "events_" + this.collectionname + "_sortby", defaultvalue: "_created"});
        if(this.collectionname == "fs.files" && this.serverOptions.sortBy == "_created") {
          this.serverOptions.sortBy = "metadata._created"
        }
  ...
  methods: {
    ...
    async DoSearch() {
      if (this.lastSearchValue != this.searchValue) {
        await this.PageStateSet({key: "events_" + this.collectionname + "_searchValue", value: this.searchValue});
        this.serverItemsLength = 0;
    ...
    async GetData() {
      ...
      try {
        ...
        if(this.serverOptions.page == 1){
          await this.PageStateSet({key: "events_" + this.collectionname + "_page", value: ""});
        } else {
          await this.PageStateSet({key: "events_" + this.collectionname + "_page", value: this.serverOptions.page});
        }
        if(this.serverOptions.rowsPerPage == 10){
          await this.PageStateSet({key: "events_" + this.collectionname + "_rowsperpage", value: ""});
        } else {
          await this.PageStateSet({key: "events_" + this.collectionname + "_rowsperpage", value: this.serverOptions.page});
        }
        if(this.serverOptions.sortBy == "_created" && this.collectionname != "fs.files") {
          await this.PageStateSet({key: "events_" + this.collectionname + "_sortby", value: ""});
        } else if(this.serverOptions.sortBy == "metadata._created" && this.collectionname == "fs.files") {
          await this.PageStateSet({key: "events_" + this.collectionname + "_sortby", value: ""});
        } else {
          await this.PageStateSet({key: "events_" + this.collectionname + "_sortby", value: this.serverOptions.sortBy});
        }
        if(this.serverOptions.sortType == "desc") {
          await this.PageStateSet({key: "events_" + this.collectionname + "_sorttype", value: ""});
        } else {
          await this.PageStateSet({key: "events_" + this.collectionname + "_sorttype", value: this.serverOptions.sortType});
        }
